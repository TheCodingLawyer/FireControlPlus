...
public class ReportCommand implements TabExecutor {
    ...
    public ReportCommand(TaskScheduler tr, ReportsManager rm, Database db, BungeeManager bm,
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        if (
            !UserUtils.checkPlayer(s)
                    || (ReportUtils.permissionRequiredToReport() && !Permission.REPORT.check(s))
        ) {
            return true;
        }
        ...
        if (
            args.length == 0
                    || (args.length == 1
                            && !ConfigUtils.exists(configFile, "Config.DefaultReasons.Reason1"))
        ) {
        ...
        LOGGER.info(
                () -> "user = " + u + ", user name = " + u.getName() + ", p = u.getPlayer() ? "
                        + (p == u.getPlayer())
        );
        ...
        u.getCooldownAsynchronously(db, taskScheduler, (cooldown) -> {
            ...
            um.getUserByNameAsynchronously(reportedName, db, taskScheduler, (ru) -> {
                ...
                LOGGER.info(
                        () -> "reported user = " + ru.getName() + ", is online: " + ru.isOnline()
                );
    ...
    private void processReportCommand(String[] args, User u, User ru, boolean reportOneself,
        ...
        if (
            ReportUtils.onlinePlayerRequired()
                    && ((rp != null && !p.canSee(rp)) || !ru.isOnlineInNetwork(bm))
        ) {
            ...
            public void onResultReceived(String reportedImmunity) {
                ...
                if (reportedImmunity != null && !reportOneself) {
                    if (User.IMMUNITY_ALWAYS.equals(reportedImmunity)) {
                        u.sendErrorMessage(
                                Message.PERMISSION_REPORT.get().replace("_Player_", ru.getName())
                        );
                    } else {
                        u.sendErrorMessage(
                ...
                if (!ConfigUtils.isEnabled(configFile, "Config.CustomReasons")) {
                    for (
                            int reasonIndex = 1;
                            reasonIndex <= MAX_DEFAULT_REASONS_AMOUNT;
                            reasonIndex++
                    ) {
                ...
                if (ReportUtils.stackReports()) {
                    LOGGER.info(
                            () -> "processReportCommand(): stackReports, checking if similar existing report"
                    );
                    ReportUtils.stackReportAsynchronously(
                            u.getUniqueId().toString(),
                            ru.getUniqueId().toString(),
                            freason,
                            date,
                            ConfigUtils.isEnabled(configFile, "Config.UpdateDateOfStackedReports"),
                            taskScheduler,
                            db,
                            rm,
                            um,
                            (result) -> {
                                if (result != null) {
                                    if (result instanceof Boolean) {
                                        if (Boolean.FALSE.equals(result)) {
                                            ...
                                            u.sendErrorMessage(
                                                    Message.get(
                                                            "ErrorMessages.Player-already-reported-by-you"
                                                    )
                                                            .replace("_Player_", ru.getName())
                                        ...
                                        LOGGER.info(
                                                () -> "processReportCommand(): stackReport succeeded"
                                        );
                                        finishReportCommandProcess(
                                                (Report) result,
                                                false,
                                                u,
                                                ru,
                                ...
                                LOGGER.info(
                                        () -> "processReportCommand(): stackReport failed, create new report"
                                );
    ...
    private void createNewReportThenFinish(User u, User ru, FileConfiguration configFile,
            String freason, String date) {
                        ...
                        if (cr.r != null) {
                            ...
                            LOGGER.error(
                                    ConfigUtils.getInfoMessage(
                                            "An error occurred while creating a report",
    ...
    private void finishReportCommandProcess(Report r, boolean missingData, User u, User ru,
        ...
        bm.sendPluginNotificationToAll(
                new NewReportBungeeNotification(
        ...
        for (String command : configFile.getStringList("Config.AutoCommands")) {
            Bukkit.dispatchCommand(
                    ...
                    command.replace("_Id_", reportIdStr).replace("_Server_", server).replace("_Date_", date).replace("_Reporter_", u.getName()).replace("_Reported_", ru.getName()).replace("_Reason_", reason)
            );
        }
    }
    ...
    public List<String> onTabComplete(CommandSender s, Command cmd, String label, String[] args) {
        return args.length == 1 && s instanceof Player
                ? StringUtil.copyPartialMatches(
                        args[0],
                        UserUtils.getOnlinePlayersForPlayer((Player) s, true, um, bm),
                        new ArrayList<>()
                )
                : new ArrayList<>();
    }
