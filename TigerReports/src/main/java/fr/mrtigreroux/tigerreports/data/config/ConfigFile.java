    ...
    public void load(TigerReports tr) {
        ...
        try {
            Reader defaultConfigStream =
                    new InputStreamReader(tr.getResource(file.getName()), "UTF8");
            ...
            Logger.CONFIG.error(
                    ConfigUtils.getInfoMessage(
                            "An error has occurred while loading config files:",
                            "Une erreur est survenue en chargeant les fichiers de configuration:"
                    ),
                    ex
            );
        }
