...
public class MySQL extends Database {
    ...
    public MySQL(String driverClassName, String connectionUrl, String host, int port,
        ...
        super(taskScheduler);
        this.driverClassName = driverClassName != null && !driverClassName.isEmpty()
                ? driverClassName
        ...
        if (connectionUrl != null && !connectionUrl.isEmpty()) {
            ...
            if (host == null || host.isEmpty() || database == null || database.isEmpty()) {
                throw new IllegalArgumentException(
                        "Database host and name cannot be null or empty."
                );
            }
            this.connectionUrl = "jdbc:mysql://" + host + ":" + port + "/" + database
                    ...
                    + (verifyServerCertificate ? "true" : "false") + "&useSSL="
                    + (useSsl ? "true" : "false");
        }
        ...
        try {
            ...
            logError(
                    ConfigUtils.getInfoMessage(
                            "The JDBC driver '" + driverClassName + "' is missing.",
                            "Le pilote JDBC '" + driverClassName + "' n'est pas installe."
                    ),
                    null
            );
            throw missing;
            ...
            logError(
                    ConfigUtils.getInfoMessage(
                            "An error has occurred during the connection to the MySQL database:",
                            "Une erreur s'est produite lors de la connexion a la base de donnees MySQL:"
                    ),
                    ex
            );
            throw ex;
