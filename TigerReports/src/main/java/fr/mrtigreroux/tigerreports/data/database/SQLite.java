...
public class SQLite extends Database {
    ...
    public static final String USERS_TABLE =
            "('uuid' TEXT NOT NULL PRIMARY KEY, 'name' TEXT, 'cooldown' TEXT, 'immunity' TEXT, 'notifications' TEXT, 'true_appreciations' INTEGER DEFAULT '0', 'uncertain_appreciations' INTEGER DEFAULT '0', 'false_appreciations' INTEGER DEFAULT '0', 'reports' INTEGER DEFAULT '0', 'reported_times' INTEGER DEFAULT '0', 'processed_reports' INTEGER DEFAULT '0')";
    public static final String REPORTS_TABLE =
    ...
    public static final String COMMENTS_TABLE =
            "('report_id' INTEGER NOT NULL, 'comment_id' INTEGER PRIMARY KEY, 'status' TEXT, 'date' TEXT, 'author' TEXT, 'message' TEXT)";
    ...
    public void openConnection() throws Exception {
            ...
            try {
                ...
                logError(
                        ConfigUtils.getInfoMessage(
                                "Failed creation of " + databaseFile.getName() + " file.",
                                "La creation du fichier " + databaseFile.getName() + " a echoue."
                        ),
                        ex
                );
                throw ex;
        ...
        try {
            ...
            logError(
                    ConfigUtils.getInfoMessage("SQLite is missing.", "SQLite n'est pas installe."),
                    null
            );
            throw missing;
            ...
            logError(
                    ConfigUtils.getInfoMessage(
                            "An error has occurred during the connection to the SQLite database:",
                            "Une erreur s'est produite lors de la connexion a la base de donnees SQLite:"
                    ),
                    ex
            );
            throw ex;
    ...
    public void updateUserName(String uuid, String name) {
        taskScheduler.runTaskAsynchronously(() -> {
            executeTransaction(() -> {
                update(
                        "INSERT OR IGNORE INTO tigerreports_users (uuid,name) VALUES (?,?)",
                        Arrays.asList(uuid, name)
                );
                update(
                        "UPDATE tigerreports_users SET name = ? WHERE uuid = ?",
                        Arrays.asList(name, uuid)
                );
            });
        });
    }
