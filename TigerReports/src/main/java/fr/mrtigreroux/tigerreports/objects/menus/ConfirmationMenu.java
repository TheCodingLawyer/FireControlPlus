...
public class ConfirmationMenu extends ReportManagerMenu {
    ...
    public Inventory onOpen() {
        ...
        inv.setItem(
                11,
                MenuRawItem.GREEN_CLAY.clone().name(Message.valueOf("CONFIRM_" + actionDisplayed).get()).lore(Message.valueOf("CONFIRM_" + actionDisplayed + "_DETAILS").get().replace("_Report_", report).split(ConfigUtils.getLineBreakSymbol())).create()
        );
        
        inv.setItem(13, r.getItem(null, vm, bm));
        inv.setItem(
                15,
                MenuRawItem.RED_CLAY.clone().name(Message.valueOf("CANCEL_" + actionDisplayed).get()).lore(Message.valueOf("CANCEL_" + actionDisplayed + "_DETAILS").get().split(ConfigUtils.getLineBreakSymbol())).create()
        );
