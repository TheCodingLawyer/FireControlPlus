...
public class UserMenu extends Menu implements UpdatedMenu, User.UserListener {
    ...
    public UserMenu(User u, User tu, ReportsManager rm, Database db, TaskScheduler taskScheduler,
    ...
    private void setReportsCategoryItem(Inventory inv, String tag, int slot, String displayName) {
        inv.setItem(
                slot,
                new CustomItem().type(Material.BOOKSHELF).name(Message.get(tag).replace("_Target_", displayName)).lore(Message.get(tag + "-details").replace("_Target_", displayName).split(ConfigUtils.getLineBreakSymbol())).create()
        );
    }
    ...
    private void updateStatisticsDisplay(Inventory inv) {
        ...
        String[] userStatDetails =
                Message.USER_STATISTIC_DETAILS.get().split(ConfigUtils.getLineBreakSymbol());
        ...
        for (Statistic stat : Statistic.values()) {
            ...
            inv.setItem(
                    stat.getPosition(),
                    stat.getCustomItem().amount(value).name(userStatMessage.replace("_Statistic_", stat.getName()).replace("_Amount_", Integer.toString(value))).lore(hasManagePerm ? userStatDetails : null).create()
            );
        }
    }
