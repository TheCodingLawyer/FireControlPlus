...
public abstract class UserReportsPageMenu extends ReportsPageMenu {
    ...
    public UserReportsPageMenu(String tag, User u, int page, User tu,
            ReportsCharacteristics reportsCharacteristics, ReportsManager rm, Database db,
    ...
    public Inventory onOpen() {
        ...
        inv.setItem(
                0,
                new CustomItem().skullOwner(name).name(Message.USER.get().replace("_Target_", displayName)).lore(Message.get("Menus.User-details").replace("_Target_", displayName).split(ConfigUtils.getLineBreakSymbol())).create()
        );
    ...
    public void onUpdate(Inventory inv) {
        ...
        if (reportsPage.characteristics.reportsCharacteristics.archived) {
            actionsBefore =
                    Message.REPORT_RESTORE_ACTION.get() + (u.hasPermission(Permission.STAFF_DELETE) ? Message.REPORT_DELETE_ACTION.get() : "");
            archiveAction = false;
            actionsAfter = "";
        } else {
            actionsBefore = Message.REPORT_SHOW_ACTION.get();
            archiveAction = u.hasPermission(Permission.STAFF_ARCHIVE);
            actionsAfter = u.hasPermission(Permission.STAFF_DELETE)
                    ? Message.REPORT_DELETE_ACTION.get()
                    : "";
        }
    ...
    private void processClickOnArchivedReport(Report r, ClickType click) {
        if (r == null) {
            ...
            u.openConfirmationMenu(
                    r,
                    ConfirmationMenu.Action.DELETE_ARCHIVE,
                    rm,
                    db,
                    taskScheduler,
                    vm,
