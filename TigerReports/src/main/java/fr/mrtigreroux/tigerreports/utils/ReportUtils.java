...
public class ReportUtils {
    ...
    /**
     * Check if a similar report already exists (same reported and reason). If it does exist, check
     * if the reporter is not already a reporter of that existing report. If it is not the case,
     * then the existing report is updated with the new reporter.
     * 
     * @param reporterUUID
     * @param reportedUUID
     * @param reason
     ...
     * @param rm
     * @param um
     * @param resultCallback return false if reporter is already a reporter of the similar existing
     *                       report, else the updated report if found and stack succeeded, else
     *                       null.
     */
    public static void stackReportAsynchronously(String reporterUUID, String reportedUUID,
            String reason, String date, boolean updateDate, TaskScheduler taskScheduler,
            Database db, ReportsManager rm, UsersManager um,
        ...
        taskScheduler.runTaskAsynchronously(() -> {
            Map<String, Object> reportData = db.query(
                    ...
                    Arrays.asList(Status.DONE.getConfigName() + "%", reportedUUID, 0, reason)
            ).getResult(0);
            if (reportData != null) {
                LOGGER.info(
                        () -> "stackReportAsynchronously(): found a similar report: "
                ...
                try {
                    ...
                    if (reportReporterUUID.contains(reporterUUID.toString())) {
                        taskScheduler.runTask(() -> {
                            LOGGER.info(
                                    () -> "stackReportAsynchronously(): player already reporter"
                            );
    ...
    public static void createReportAsynchronously(User u, User ru, String reason, String date,
            boolean maxReportsReached, TaskScheduler taskScheduler, Database db, BungeeManager bm,
            UsersManager um, ResultCallback<CreatedReport> resultCallback) {
        ...
        if (!maxReportsReached) {
            ...
            String query = "INSERT INTO tigerreports_reports (" + queryColumnsName + ") VALUES ("
                    + queryColumnsValue + ")";
                    ...
                    new ResultCallback<Integer>() {
                        ...
                        public void onResultReceived(Integer reportId) {
                            ...
                            Report.asynchronouslyFrom(
                                    ...
                                    (r) -> {
                                        LOGGER.debug(
                                                () -> "onResultReceived(): send report result"
                                        );
                                        resultCallback.onResultReceived(
                                                new CreatedReport(r, missingData)
                                        );
                                    }
                            );
                        }
    ...
    public static boolean collectAndFillReportedData(User ru, BungeeManager bm,
        ...
        try {
            ...
            data.put(
                    ...
                    AdvancedData.serializeMessages(ru.getLastMessages())
            );
            data.put(
                    AdvancedData.REPORTED_GAMEMODE,
                    AdvancedData.serializeGamemode(rp.getGameMode())
            );
            data.put(
            ...
            data.put(
                    AdvancedData.REPORTED_HEALTH,
                    (int) Math.round(rp.getHealth()) + "/" + (int) Math.round(rp.getMaxHealth())
            );
            data.put(AdvancedData.REPORTED_FOOD, rp.getFoodLevel());
            data.put(
                    AdvancedData.REPORTED_EFFECTS,
                    AdvancedData.serializeConfigEffects(rp.getActivePotionEffects())
            );
            return true;
        } catch (Exception ex) {
            return false;
    ...
    @SuppressWarnings("deprecation")
    public static void sendReport(Report r, String server, boolean notify, Database db,
        ...
        alert.setColor(
                ChatColor.valueOf(MessageUtils.getLastColor(Message.ALERT.get(), "_Reason_").name())
        );
        if (reportId == -1) {
            MessageUtils.sendStaffMessage(
                    Message.STAFF_MAX_REPORTS_REACHED.get()
                            .replace("_Amount_", Integer.toString(getMaxReports())),
                    ConfigSound.STAFF.get()
            );
        } else {
            alert.setClickEvent(
                    new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/reports #" + reportId)
            );
        ...
        String[] lines = Message.ALERT.get()
                ...
                .replace(
                        ...
                        r.getPlayerName(
                                ...
                                !ReportUtils.onlinePlayerRequired(),
                                true,
                                vm,
                                bm
                        )
        ...
        for (String line : lines) {
            ...
            MessageUtils.APPEND_TEXT_WITH_TRANSLATED_COLOR_CODES_TO_COMPONENT_BUILDER_METHOD
                    .accept(alertLine, line);
            MessageUtils.sendStaffMessage(alertLine, ConfigSound.REPORT.get());
        }
    }
    
    public static void checkMaxReportsReachedAsynchronously(TaskScheduler taskScheduler,
            Database db, ResultCallback<Boolean> resultCallback) {
        taskScheduler.runTaskAsynchronously(() -> {
    ...
    public static int getTotalReports(Database db) {
        Object o = db.query("SELECT COUNT(report_id) AS total FROM tigerreports_reports", null)
                .getResult(0, "total");
