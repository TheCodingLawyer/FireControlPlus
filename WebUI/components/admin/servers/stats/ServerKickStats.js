...
const colour = fullConfig.theme.colors.orange['800']

const query = `
query serverKickStats($id: ID!, $intervalDays: Int!) {
  serverKickStats(id: $id, intervalDays: $intervalDays) {
    total
    totalHistory {
      date
      value
...
export default function ServerKickStats ({ server }) {
  const [intervalDays, setIntervalDays] = useState(7)
  const { loading, data, errors } = useApi({ query, variables: { id: server.id, intervalDays } })

  if (errors) return null

  const history = (data?.serverKickStats?.totalHistory || []).map(row => ({
    x: format(fromUnixTime(row.date), 'd MMM yyyy'),
    y: row.value
  }))

  const safeHistory = history.length ? history : [{ x: format(new Date(), 'd MMM yyyy'), y: 0 }]
  ...
  return (
    <div className='pt-5 bg-black shadow-md rounded-md w-full'>
      <div className='px-5 flex justify-between items-center text-sm text-gray-500'>
        <h5 className='uppercase'>Kicks</h5>
        <Dropdown
        ...
        {data &&
          <>
            <div className='text-center'>
              <h2 className='title-font font-medium text-xl'>{data.serverKickStats.total}</h2>
