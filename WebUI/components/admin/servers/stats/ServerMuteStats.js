...
const colour = fullConfig.theme.colors.indigo['500']

const query = `
query serverMuteStats($id: ID!, $intervalDays: Int!) {
  serverMuteStats(id: $id, intervalDays: $intervalDays) {
    total
    averageLength
    totalHistory {
      date
...
export default function ServerMuteStats ({ server }) {
  ...
  const history = (data?.serverMuteStats?.totalHistory || []).map(row => ({
