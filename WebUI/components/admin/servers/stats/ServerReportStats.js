...
export default function ServerReportStats ({ server }) {
  ...
  const history = (data?.serverReportStats?.totalHistory || []).map(row => ({
    x: format(fromUnixTime(row.date), 'd MMM yyyy'),
    y: row.value
  }))
  let safeHistory = history.length ? history : [{ x: format(new Date(), 'd MMM yyyy'), y: 0 }]
  // Ensure at least two points to keep line/area rendering consistent
  if (safeHistory.length === 1) {
    safeHistory = [
      { x: format(subDays(new Date(), 1), 'd MMM yyyy'), y: 0 },
      safeHistory[0]
    ]
  }
