...
const colour = fullConfig.theme.colors.amber['500']

const query = `
query serverWarningStats($id: ID!, $intervalDays: Int!) {
  serverWarningStats(id: $id, intervalDays: $intervalDays) {
    total
    averageLength
    totalHistory {
      date
...
export default function ServerWarningStats ({ server }) {
  ...
  const history = (data?.serverWarningStats?.totalHistory || []).map(row => ({
