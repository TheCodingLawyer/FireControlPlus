...
import PlayerKickForm from '../PlayerKickForm'
...
const PunishmentAction = ({ children, type, report, server, onAction }) => {
  ...
  <PlayerKickForm
    serverFilter={s => s.id === server.id}
    query={createQuery('resolveReportKick', 'CreatePlayerKickInput')}
    submitRef={submitRef}
    parseVariables={(input) => ({
      report: report.id,
      serverId: server.id,
    ...
    onFinished={({ resolveReportKick }) => {
      setOpen(false)
      onAction(resolveReportKick)
    }}
  />
          }[type]
        }
...
export default function PlayerReportActions ({ report, server, onAction }) {
  ...
  const canCreateKick = hasServerPermission('player.kicks', 'create', server.id)
    ...
    <div className='flex flex-col gap-6'>
      ...
      {canCreateKick &&
        <PunishmentAction
          ...
          onAction={onAction}
        >
          <Button className='bg-orange-800'><FaSignOutAlt className='text-xl mr-2' /> Kick</Button>
        </PunishmentAction>}
      {canCreateWarning &&
        <PunishmentAction
          type='Warn'
