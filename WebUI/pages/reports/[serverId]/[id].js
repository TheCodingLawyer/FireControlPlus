...
import PlayerReportChatHistory from '../../../components/reports/PlayerReportChatHistory'
...
export default function Page () {
  ...
  const reporter = report.actor?.name || 'Unknown'

  return (
    <DefaultLayout title={`#${id} | ${report.reason} | Report`}>
      <PageContainer>
        <div className='pb-6'>
          <h1
            className='text-2xl font-bold break-words pb-2 flex items-center gap-3'
          >
            <span>{report.reason}</span>
            <span className='block md:inline text-gray-400'>#{report.id}</span>
            {report.provider && (
              <span className='ml-2 px-2 py-0.5 rounded-full text-xs bg-primary-800 text-gray-200'>
                {report.provider === 'tigerreports' ? 'TigerReports' : 'BanManager'}
              </span>
            )}
          </h1>
          <p className='pb-4 border-b border-accent-400 text-gray-400'>
            {report.actor?.id ? (
              <Link href={`/player/${report.actor.id}`}>{reporter}</Link>
            ) : reporter}
            &nbsp;reported&nbsp;
            <Link href={`/player/${report.player.id}`}>{report.player.name}</Link>
        ...
        <div className='grid grid-flow-row md:grid-flow-col grid-cols-12'>
          <div className='col-span-12 md:col-span-9 space-y-6'>
            ...
            <div>
              <PlayerReportChatHistory report={report} serverId={serverId} />
            </div>
            <div className='flex'>
