...
module.exports = async function serverKickStats (obj, { id, intervalDays }, { state: { serversPool }, log }) {
  ...
  const { total } = await pool(config.tables.playerKicks)
    .select(pool.raw('COUNT(*) AS `total`'))
    .where('created', '>=', startUnixTime)
    .first()

  const totalHistoryValues = await pool(config.tables.playerKicks)
