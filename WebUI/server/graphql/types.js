...
type PlayerReportList {
  total: Int! @cacheControl(scope: PUBLIC, maxAge: 10)
  records: [PlayerReport!]! @cacheControl(scope: PUBLIC, maxAge: 10)
  server: Server!
}
...
type PlayerAppealList {
  total: Int! @cacheControl(scope: PUBLIC, maxAge: 10)
  records: [PlayerAppeal!]! @cacheControl(scope: PUBLIC, maxAge: 10)
  server: Server!
}
...
type PlayerBanList {
  total: Int! @cacheControl(scope: PUBLIC, maxAge: 10)
  records: [PlayerBan!]! @cacheControl(scope: PUBLIC, maxAge: 10)
}

type PlayerMuteList {
  total: Int! @cacheControl(scope: PUBLIC, maxAge: 10)
  records: [PlayerMute!]! @cacheControl(scope: PUBLIC, maxAge: 10)
}

type PlayerWarningList {
  total: Int! @cacheControl(scope: PUBLIC, maxAge: 10)
  records: [PlayerWarning!]! @cacheControl(scope: PUBLIC, maxAge: 10)
}
...
type PlayerChatHistory @sqlTable(name: "playerChatHistory") {
  id: ID!
  message: String!
  world: String!
  x: Int!
...
union PlayerPunishmentRecord = PlayerBan | PlayerBanRecord | PlayerKick | PlayerMute | PlayerMuteRecord | PlayerNote | PlayerWarning
...
type Query {
  ...
  serverKickStats(id: ID!, intervalDays: Int!): ServerPunishmentStats! @allowIf(resource: "servers", permission: "manage")
  ...
  listPlayerPunishmentRecords(serverId: ID!, actor: UUID, player: UUID, type: RecordType!, order: OrderByInput): PlayerPunishmentRecords! @cacheControl(scope: PRIVATE, maxAge: 10)
  ...
  playerChatHistory(playerId: UUID!, serverId: ID!, limit: Int = 50): [PlayerChatHistory!] @allowIf(resource: "player.reports", permission: "view.any", serverVar: "serverId")
  ...
  statistics: Statistics! @cacheControl(scope: PUBLIC, maxAge: 1)
...
type Mutation {
  ...
  createPlayerKick(input: CreatePlayerKickInput!): PlayerKick @allowIf(resource: "player.kicks", permission: "create", serverVar: "input.server")
