...
public class BanManagerIntegration {
    ...
    public static boolean banPlayer(String actorUUID, String actorName, String targetUUID, String targetName, String reason, Date expires, boolean silent) {
        ...
        try {
            ...
            if (expires == null) {
                // Permanent ban
                command = silent ? 
                    "bmsilentban " + targetName + " " + reason :
                    "ban " + targetName + " " + reason;
            } else {
                // Temporary ban - calculate duration in seconds
                long durationSeconds = (expires.getTime() - System.currentTimeMillis()) / 1000;
                command = silent ? 
                    "bmsilentban " + targetName + " " + durationSeconds + "s " + reason :
                    "ban " + targetName + " " + durationSeconds + "s " + reason;
            }
    ...
    public static boolean mutePlayer(String actorUUID, String actorName, String targetUUID, String targetName, String reason, Date expires, boolean silent) {
        ...
        try {
            ...
            if (expires == null) {
                // Permanent mute
                command = silent ? 
                    "bmsilentmute " + targetName + " " + reason :
                    "mute " + targetName + " " + reason;
            } else {
                // Temporary mute - calculate duration in seconds
                long durationSeconds = (expires.getTime() - System.currentTimeMillis()) / 1000;
                command = silent ? 
                    "bmsilentmute " + targetName + " " + durationSeconds + "s " + reason :
                    "mute " + targetName + " " + durationSeconds + "s " + reason;
            }
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command);
            return true;
        } catch (Exception e) {
    ...
    public static boolean kickPlayer(String actorUUID, String actorName, String targetUUID, String targetName, String reason) {
        ...
        try {
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "kick " + targetName + " " + reason);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public static boolean warnPlayer(String actorUUID, String actorName, String targetUUID, String targetName, String reason, Date expires, boolean silent) {
        ...
        try {
            String command = silent ? 
                "bmsilentwarn " + targetName + " " + reason :
                "warn " + targetName + " " + reason;
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
