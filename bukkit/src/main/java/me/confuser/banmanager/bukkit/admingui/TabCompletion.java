...
import me.confuser.banmanager.bukkit.admingui.utils.Language;
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
		...
		if (!(sender instanceof Player)) {
			if (command.getName().equalsIgnoreCase("admin")) {
				...
				if (args.length == 1) {
					...
					if(AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false)) completions.add("rank");
					if(AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false)) completions.add("permission");
					...
					if (args[0].equals("rank") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false)) {
						completions.add("set");
						completions.add("up");
						completions.add("down");
					} else if (args[0].equals("permission") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false)){
						completions.add("add");
						...
						for (String uuid_name : con_sec) {
							completions.add(AdminGuiIntegration.getInstance().getPlayers().getString(uuid_name + ".name"));
						}
					}
				} else if (args.length == 3) {
					if(args[0].equals("permission") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false) && (args[1].equals("add") || args[1].equals("remove"))) {
						for (Player all : Bukkit.getServer().getOnlinePlayers()) completions.add(all.getName());
					} else if (args[0].equals("rank") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false) && (args[1].equals("set") || args[1].equals("up") || args[1].equals("down"))) {
						if (AdminGuiIntegration.getInstance().getConf().getBoolean("bungeecord_enabled", false)) {
							Channel.send(sender.getName(), "send", "online_players");
							completions.addAll(Settings.online_players);
						} else {
					...
					if(args[0].equals("permission") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false) && args[1].equals("remove")) {
						Player target_player = Bukkit.getServer().getPlayer(args[2]);
						if(target_player != null) {
							List<String> playerPermissions = AdminGuiIntegration.getInstance().getPlayers().getStringList(target_player.getUniqueId() + ".permissions");
							completions.addAll(playerPermissions);
						}
					} else if (args[0].equals("rank") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false) && args[1].equals("set")) {
						for (Map.Entry<String, Object> rank : AdminGuiIntegration.getInstance().getPermissions().getConfigurationSection("groups").getValues(false).entrySet()) {
		...
		if (command.getName().equalsIgnoreCase("admin")) {
			...
			if (args.length == 1) {
				...
				if (TargetPlayer.hasPermission(player, "admingui.rank") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false))
					completions.add("rank");

				if(TargetPlayer.hasPermission(player, "admingui.permission.manage") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false))
				...
				} else if (args[0].equals("rank") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false)) {
				...
				if(args[0].equals("permission") && AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false) && args[1].equals("remove") && TargetPlayer.hasPermission(player, "admingui.permission.manage")) {
					Player target_player = Bukkit.getServer().getPlayer(args[2]);
					if(target_player != null) {
						List<String> playerPermissions = AdminGuiIntegration.getInstance().getPlayers().getStringList(target_player.getUniqueId() + ".permissions");
						completions.addAll(playerPermissions);
