package me.confuser.banmanager.bukkit.admingui.commands;
		...
		if (!(sender instanceof Player)) {
			...
			if (args.length == 1) {
				if (args[0].equals("reload")) {
					...
					AdminGuiIntegration.getInstance().mkdir();
					AdminGuiIntegration.getInstance().loadYamls();
				...
				if (args[0].equals("rank")) {
					...
					for (String uuid_name : con_sec) {
						if (AdminGuiIntegration.getInstance().getPlayers().getString(uuid_name + ".name", "").equals(name)) {
							...
							if (AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false)) {
								sender.sendMessage(Message.chat("  &9Rank: &b" + AdminGuiIntegration.getInstance().getPlayers().getString(uuid_name + ".rank", "default")));
							}
							StringBuilder ips = new StringBuilder();
							for (String ip : AdminGuiIntegration.getInstance().getPlayers().getStringList(uuid_name + ".ips")) {
							...
							for (String uuid_name2 : con_sec) {
								String alt_name = AdminGuiIntegration.getInstance().getPlayers().getString(uuid_name2 + ".name");
								for (String ip : AdminGuiIntegration.getInstance().getPlayers().getStringList(uuid_name2 + ".ips")) {
							...
							sender.sendMessage(Message.chat("  &9Last Join: &b" + Permissions.Database.date_format.format(new Date(AdminGuiIntegration.getInstance().getPlayers().getLong(uuid_name + ".lastJoin")))));
							sender.sendMessage(Message.chat("  &9First Join: &b" + Permissions.Database.date_format.format(new Date(AdminGuiIntegration.getInstance().getPlayers().getLong(uuid_name + ".firstJoin")))));
							break;
						}
					}
				}
				...
				if (args[0].equals("permission")){
					...
					if (args[1].equals("up")) {
						...
						if (target_player != null) {
							...
							int cur_priority = AdminGuiIntegration.getInstance().getPermissions().getInt("groups." + cur_rank + ".priority", AdminGuiIntegration.getInstance().getPermissions().getInt("groups.default.priority"));
							ArrayList<Integer> priorities = new ArrayList<>();
							for (Map.Entry<String, Object> priority : AdminGuiIntegration.getInstance().getPermissions().getConfigurationSection("groups").getValues(false).entrySet()) {
								int pri = AdminGuiIntegration.getInstance().getPermissions().getInt("groups." + priority.getKey() + ".priority");
								if (cur_priority > pri) priorities.add(pri);
							}
							if (priorities.size() > 0) {
								int max_pri = Collections.max(priorities);
								for (Map.Entry<String, Object> priority : AdminGuiIntegration.getInstance().getPermissions().getConfigurationSection("groups").getValues(false).entrySet()) {
									if (AdminGuiIntegration.getInstance().getPermissions().getInt("groups." + priority.getKey() + ".priority") == max_pri) {
						...
						if (target_player != null) {
							...
							if (priorities.size() > 0) {
								...
								for (Map.Entry<String, Object> priority : AdminGuiIntegration.getInstance().getPermissions().getConfigurationSection("groups").getValues(false).entrySet()) {
									if (AdminGuiIntegration.getInstance().getPermissions().getInt("groups." + priority.getKey() + ".priority") == min_pri) {
										Permissions.setRank(target_player.getUniqueId(), target_player.getName(), priority.getKey());
										TargetPlayer.refreshPlayerTabList(target_player);
										TargetPlayer.refreshPermissions(target_player);
										sender.sendMessage(Message.getMessage(UUID.randomUUID(), "prefix") + Message.getMessage(UUID.randomUUID(), "message_admin_rank").replace("{player}", target_player.getName()).replace("{rank}", priority.getKey()));
		...
		if (TargetPlayer.hasPermission(player, "admingui.admin")) {
			...
			if (args.length == 0) {
				...
				if (args[0].equals("reload")) {
					...
					if (TargetPlayer.hasPermission(player, "admingui.admin") && AdminGuiIntegration.getInstance().getConf().getBoolean("admin_tools_enabled", true)) {
					...
					} else if (AdminGuiIntegration.getInstance().getConf().getBoolean("bungeecord_enabled", false) && Settings.online_players.contains(ChatColor.stripColor(args[0]))) {
						//TODO: Bungee
					...
					if (TargetPlayer.hasPermission(player, "admingui.check")) {
						...
						for (String uuid_name : con_sec) {
							if (AdminGuiIntegration.getInstance().getPlayers().getString(uuid_name + ".name").equals(name)) {
								...
								if (AdminGuiIntegration.getInstance().getConf().getBoolean("ap_enabled", false)) {
									player.sendMessage(Message.chat("  &9Rank: &b" + AdminGuiIntegration.getInstance().getPlayers().getString(uuid_name + ".rank", "default")));
								}
								StringBuilder ips = new StringBuilder();
								for (String ip : AdminGuiIntegration.getInstance().getPlayers().getStringList(uuid_name + ".ips")) {
									ips.append(ip).append(", ");
								...
								player.sendMessage(Message.chat("  &9Last Join: &b" + Permissions.Database.date_format.format(new Date(AdminGuiIntegration.getInstance().getPlayers().getLong(uuid_name + ".lastJoin")))));
								player.sendMessage(Message.chat("  &9First Join: &b" + Permissions.Database.date_format.format(new Date(AdminGuiIntegration.getInstance().getPlayers().getLong(uuid_name + ".firstJoin")))));
								break;
							}
						}
					} else {
