...
public class PlayerPlaceholderMessageListener implements Listener {
	...
	public PlayerPlaceholderMessageListener(AdminGuiIntegration plugin) {
		adminGUI = plugin;
	...
	public void onPlayerPlaceholderChat(AsyncPlayerChatEvent event) {
		...
		if (Settings.freeze.getOrDefault(p.getUniqueId(), false) && AdminGuiIntegration.getInstance().getConf().getBoolean("freeze_send_message", true)) {
			event.setCancelled(true);

			String freeze_channel = AdminGuiIntegration.getInstance().getConf().getString("freeze_admin_chat", null);
		...
		if (!command.equals("")) {
			...
			if (adminGUI.getConf().getBoolean("ac_enabled", false)) {
				...
				if (Settings.muted_chat && !TargetPlayer.hasPermission(p, "AdminGuiIntegration.chat.mute.bypass")) {
					event.setCancelled(true);
					p.sendMessage(Message.getMessage(p.getUniqueId(), "message_admin_chat_muted"));
					return;
				}

				// BanManager mute checking is handled by the plugin itself
				// No additional mute check needed here

				if (adminGUI.getConf().getDouble("ac_delay", 0) > 0 && !TargetPlayer.hasPermission(p, "AdminGuiIntegration.chat.delay.bypass")) {
				...
				if (!TargetPlayer.hasPermission(p, "AdminGuiIntegration.chat.color") && !TargetPlayer.hasPermission(p, "AdminGuiIntegration.chat.colors")) {
				...
				if (!TargetPlayer.hasPermission(p, "AdminGuiIntegration.chat.advertisement.bypass")) {
					if (!message.endsWith(".")) {
				...
				if (!TargetPlayer.hasPermission(p, "AdminGuiIntegration.chat.filter.bypass")) {
				...
				if (AdminGuiIntegration.getVaultChat() != null) {
					vault_prefix = AdminGuiIntegration.getVaultChat().getPlayerPrefix(p);
					vault_suffix = AdminGuiIntegration.getVaultChat().getPlayerSuffix(p);
				}
