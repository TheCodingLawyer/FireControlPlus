	...
	public String onRequest(OfflinePlayer offlinePlayer, String identifier) {
		...
		switch (identifier) {
			...
			case "player_first_join":
				return Permissions.Database.date_format.format(new Date(AdminGuiIntegration.getInstance().getPlayers().getLong(player.getUniqueId() + ".firstJoin")));
			case "player_last_join":
				return Permissions.Database.date_format.format(new Date(AdminGuiIntegration.getInstance().getPlayers().getLong(player.getUniqueId() + ".lastJoin")));
			case "player_ips":
				StringBuilder ips = new StringBuilder();
				for (String ip : AdminGuiIntegration.getInstance().getPlayers().getStringList(player.getUniqueId() + ".ips")) {
