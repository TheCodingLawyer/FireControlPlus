...
import me.confuser.banmanager.bukkit.util.FloodgateUtil;
...
public class JoinListener implements Listener {
  ...
  public void banCheck(final AsyncPlayerPreLoginEvent event) {
    FloodgateUtil.FloodgateIdentity bedrock = FloodgateUtil.remapIdentity(event.getUniqueId(), event.getName());
    if (bedrock != null) {
      listener.banCheck(bedrock.uuid, bedrock.name, IPUtils.toIPAddress(event.getAddress()), new BanJoinHandler(plugin, event));
    } else {
  ...
  public void onJoin(AsyncPlayerPreLoginEvent event) {
    ...
    FloodgateUtil.FloodgateIdentity bedrock = FloodgateUtil.remapIdentity(event.getUniqueId(), event.getName());
    if (bedrock != null) {
      listener.onPreJoin(bedrock.uuid, bedrock.name, IPUtils.toIPAddress(event.getAddress()));
    } else {
