...
/**
 * User Reports Menu - Shows all reports made BY a specific player
 * Layout: 54 slots (6 rows)
 * Features: Player head, report list, pagination
 */
public class UserReportsMenu extends Menu implements UpdatedMenu {
    ...
    public UserReportsMenu(User u, int page, String targetPlayerName, ReportsManager rm, 
    ...
    public Inventory onOpen() {
        Inventory inv = getInventory("§6Reports by §e" + targetPlayerName, true);
    ...
    private ItemStack createPlayerHead() {
        ...
        if (meta instanceof SkullMeta) {
            ...
            skullMeta.setDisplayName("§6Player: §e" + targetPlayerName);
            skullMeta.setLore(Arrays.asList(
                "§7Viewing reports made by this player",
                "",
    ...
    private ItemStack createPageInfoButton() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.setDisplayName("§6Reports by " + targetPlayerName);
    ...
    private void loadReports() {
        // Load reports made by this player
        rm.getReportsByPlayerAsynchronously(targetPlayerName, true, page, 27, db, tr, um, new ResultCallback<ReportsPage>() {
            @Override
        ...
        switch (slot) {
            ...
            case 45: // Previous page
                if (reportsPage != null && reportsPage.hasPreviousPage()) {
                    u.openUserReportsMenu(page - 1, targetPlayerName, rm, db, tr, vm, bm, um);
                }
                break;
