...
import me.confuser.banmanager.bungee.util.FloodgateUtil;
...
public class JoinListener implements Listener {
  ...
  public void banCheck(LoginEvent event) {
    ...
    plugin.getPlugin().getScheduler().runAsync(() -> {
      ...
      listener.banCheck(uuid, name, IPUtils.toIPAddress(event.getConnection().getAddress().getAddress()), new BanJoinHandler(plugin.getPlugin(), event));

      if (!event.isCancelled()) {
        listener.onPreJoin(uuid, name, IPUtils.toIPAddress(event.getConnection().getAddress().getAddress()));
      }
